...
// Offers — public page
// ----------------------------------------------------------------------------
// Reads offers from the Supabase `offers` table (managed via /analytics CMS).
// Falls back to nothing if the table is empty — admin should populate it.
// ============================================================================
...
import { fetchOffers, type Offer } from '../lib/cms';
          ...
          {loading ? (
            ...
            >
              {offers.map((offer, index) => {
                const title = pickText(offer.title_en, offer.title_ar);
                const subtitle = pickText(offer.subtitle_en, offer.subtitle_ar);
                const price = pickText(offer.price_en, offer.price_ar);
                const discount = pickText(offer.discount_en, offer.discount_ar);
                const popularLabel = pickText(offer.popular_label_en, offer.popular_label_ar);
                const features = (isRTL ? offer.features_ar : offer.features_en) ?? [];
                const message = offer.whatsapp_message ||
                  `Hello! I'm interested in the *${title}* offer.`;
                    ...
                    <div className={`p-6 flex-1 flex flex-col ${isRTL ? 'text-right' : 'text-left'}`}>
                      <div className="mb-6">
                        <div className={`flex items-center gap-2 mb-3 flex-wrap ${isRTL ? 'flex-row-reverse' : ''}`}>
                          {discount && (
                            <span className="bg-red-100 text-red-600 px-3 py-1 rounded-full text-xs font-semibold">{discount}</span>
                          )}
                          {offer.popular && popularLabel && (
                            <span className="bg-red-600 text-white px-3 py-1 rounded-full text-xs font-semibold flex items-center gap-1">
                              <Sparkles className="w-3 h-3" /> {popularLabel}
                            </span>
                          )}
                        </div>
                      ...
                      <div className="space-y-3 mt-auto">
                        ...
                        <button
                          onClick={() => trackAndOpenWhatsApp(title, subtitle, message, index)}
